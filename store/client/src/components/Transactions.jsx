...
const TransactionsCard = ({ addressTo, addressFrom, timestamp, message, amount }) => {
  ...
  const jpgUrl = message.endsWith('.jpg')? '/images/' + message: '/images/' + message + '.jpg';
      ...
      <div className="flex flex-col items-center w-full mt-3">
        <div className="display-flex justify-start w-full mb-6 p-2">
          ...
          {message && (
            <>
              <br />
              <p className="text-white text-base">{message.length > 32? message.substring(0,30) + '...': message}</p>
            </>
          )}
        </div>
        <img
